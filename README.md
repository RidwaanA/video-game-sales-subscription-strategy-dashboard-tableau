...
## Dashboard Features
- 🎛️ **Dynamic Top N filtering** — Parameters allow users to interactively control how many top games, platforms, or genres are displayed across visuals (by global sales, ratings, or user count)
- 🎛️ **3-in-1 dimension switcher** — A single parameter toggles between Games, Platforms, and Genres views across relevant charts, eliminating dashboard clutter
- 🎛️ **Unified ratings selector** — A calculated field consolidates User Score, Critic Score, and User Count into a single switchable metric for flexible performance comparison
- 🎛️ **Multi-level filters** — Name, category action, genre, and platform filters enable granular drill-down across all dashboard views
